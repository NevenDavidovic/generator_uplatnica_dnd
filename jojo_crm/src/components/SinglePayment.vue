<template>
  <div class="">
    ...
    >
      <button
        @click="resetForm"
        class="no-print flex items-center justify-center gap-2 bg-gray-600 text-white py-3 px-6 rounded-lg hover:bg-gray-700 transition-colors duration-200 shadow-sm hover:shadow-md active:transform active:scale-95"
      >
        Resetiraj Podatke
      </button>
      <button
...
export default {
  ...
  data() {
    return {
      sending: false, // Add this for email sending state
      paymentParams: {
        bazniKod: "HRVHUB30\nEUR",
        iznosTransakcije: "",
        imePlatitelja: "",
  ...
  mounted() {
    ...
    const organizationId = this.organizationStore.organization?.id;
    if (organizationId) {
      this.postavkeStore.fetchEmailSettings(organizationId);
    }
  },

  watch: {
    "slipStore.selectedContact": {
      immediate: true,
      handler(newContact) {
        if (newContact) {
          console.log("newContact", newContact);
          this.paymentParams.imePlatitelja = newContact.name;
          this.paymentParams.adresaPlatitelja = newContact.address;
          this.paymentParams.postanskiBrojIMjestoPlatitelja =
            newContact.postalCode + " " + newContact.city;
          this.paymentParams.iznosTransakcije = newContact.amount;
          this.paymentParams.opisPlacanja = newContact.paymentDescription;
          this.paymentParams.modelPlacanja = newContact.paymentModel;
          this.paymentParams.email = newContact.email;
