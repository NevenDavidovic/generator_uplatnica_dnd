...
export default {
  ...
  watch: {
    // Watch for changes in contact and organization props
    contact: {
      immediate: true,
      deep: true,
      handler() {
  ...
  methods: {
    setPaymentData() {
      ...
      this.paymentParams.postanskiBrojIMjestoPlatitelja =
        `${this.contact.postanski_broj} ${this.contact.grad}` || "";
    ...
    setOrganizationData() {
      ...
      this.paymentParams.imePrimatelja = this.organization.naziv || "";
      this.paymentParams.adresaPrimatelja = this.organization.ulica || "";
      this.paymentParams.postanskiBrojIMjestoPrimatelja =
        this.organization.grad || "";
      this.paymentParams.ibanPrimatelja = this.organization.IBAN || "";
    },

    concatenateStrings() {
      this.concatenatedString = Object.values(this.paymentParams)
        .map((value) => `${value}\n`)
        .join("");
    },
